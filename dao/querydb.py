...
class QueryDB(object):
    ...
    def update_ovsdb(self, ovsdb):
        ...
        res = self.db.update_sql(sql, (ovsdb.IP, ovsdb.ovsdb_addr, dpid, ovsdb.br_name,
    ...
    def get_tunnel_port(self, ip, port_name):
        ...
        res = self.db.query_sql(sql, (ip, port_name))
        if len(res) == 0:
            return None
    ...
    def update_tunnel_port(self, tunnel_port):
        ...
        res = self.db.update_sql(sql, (tunnel_port.IP,
    ...
    def delete_tunnel_port(self, ip, port_name):
        sql = " DELETE FROM tunnel_port WHERE IP=%s AND port_name = %s"
        print()
        res = self.db.execute_sql(sql, (ip, port_name))
        print(res)
